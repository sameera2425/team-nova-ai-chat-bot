    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="bg-white w-full z-20 sticky top-0 left-0 border-b border-blue-100 px-6 py-2 flex items-center justify-between">
          <p className="text-lg text-gray-600 animate-fade-in font-bold">
        ...
        <div className="max-w-2xl mx-auto mt-8 space-y-8">
          ...
          <Card title="Priority Level">
            <div className="flex gap-6">
              {priorities.map((p) => (
                <label key={p.label} className={`cursor-pointer flex items-center gap-2 px-4 py-2 rounded-lg border transition-all duration-200 ${priority === p.label ? `bg-${p.color}-100 border-${p.color}-600 text-${p.color}-900 font-semibold` : 'bg-white border-gray-200 text-gray-700 hover:bg-blue-50'}`}>
                  <input type="radio" name="goal-priority" className="hidden" checked={priority === p.label} onChange={() => setPriority(p.label)} />
